...
const OfferList: FC<OfferListProps> = ({ data }) => {
    ...
    const { fetchAllOffers, SortOffersByPrice, SortOffersByRate } =
        useActions();
    ...
    const updateSelector = (i: number) => {
        const arr = [false, false];
        const index = selectors.indexOf(true);
        if (index === -1) {
            arr[i] = !arr[i];
        } else {
            if (index !== i) {
    ...
    if (BankName !== "Любое") {
        new_data = new_data.filter((el) => {
            if (el.data.organization.name === BankName) return el;
            return undefined;
        });
    }
    ...
    return (
        <div className='listCont'>
            ...
            <div className='list'>
                {new_data.map((el, i) => {
                    return <OfferCard key={i} id={el.id} offer={el.data} />;
                })}
                {!isAllLoaded && (
                    <div className='btn' onClick={fetchAllOffers}>
                        Показать остальное
                    </div>
                )}
            </div>
