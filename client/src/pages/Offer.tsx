    ...
    return (
        <div className="cont">
            <div className="row row_gray">
                <img className="mb" src={offer.data.organization.logo} alt="logo" />
            </div>
            <div className="row row_white">
                <p>Организация: {offer.data.organization.name}</p>
                <div className="subTitle">Лицензия: №{offer.data.organization.license}</div>
            </div>
            <div className="row row_gray">
                <b>Ставка:
                    {offer.data.rate.periods[0].rate.from === offer.data.rate.periods[0].rate.to
                        ? ` ${offer.data.rate.periods[0].rate.from}%`
                        : ` от ${offer.data.rate.periods[0].rate.from}%`}
                </b>
                <div className="subTitle">
                    Программа: «{offer.data.name}»
                </div>
            </div>
            <div className="row row_white">
                <div className="money">
                ...
                <div className="subTitle">На срок до {offer.data.rate.periods[0].term.to / 12} лет</div>
            </div>
            <div className="row row_gray">
                <p className="subTitle">В возрасте от {offer.data.customerRequirements.age} лет</p>
                <p className="subTitle">{offer.data.customerRequirements.documents} документа</p>
            </div>
        </div>
    )
}
